                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200 text-sm">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @if(isset($sssTable) && $sssTable->count() > 0)
                                @foreach($sssTable as $row)
                                    <tr>
                                        <td class="px-2 py-1 text-xs">{{ $row->getDisplayRange() }}</td>
                                        <td class="px-2 py-1 text-xs">₱{{ number_format($row->msc, 2) }}</td>
                                        <td class="px-2 py-1 text-xs">₱{{ number_format($row->employee_share, 2) }}</td>
                                        <td class="px-2 py-1 text-xs">₱{{ number_format($row->employer_share, 2) }}</td>
                                        <td class="px-2 py-1 text-xs">₱{{ number_format($row->total_contribution, 2) }}</td>
                                    </tr>
                                @endforeach
                            @else
                                <tr><td colspan="5" class="px-2 py-1 text-xs text-center">No SSS table data available</td></tr>
                            @endif
                        </tbody>
                    </table>
                </div>
