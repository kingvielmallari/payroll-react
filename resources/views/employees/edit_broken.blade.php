<x-app-layout>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function updateCalculationDisplay(hourly, daily, weekly, semiMonthly, monthly) {
                if (calcHourly) calcHourly.textContent = formatPeso(hourly || 0);
                if (calcDaily) calcDaily.textContent = formatPeso(daily || 0);
                if (calcWeekly) calcWeekly.textContent = formatPeso(weekly || 0);
                if (calcSemi) calcSemi.textContent = formatPeso(semiMonthly || 0);
                if (calcMonthly) calcMonthly.textContent = formatPeso(monthly || 0);
            }
            ...
            function clearOtherFields(currentField) {
                ...
                const fields = ['hourly', 'daily', 'weekly', 'semi', 'basic'];
                const inputs = {
                    'hourly': {input: hourlyRateInput, raw: hourlyRateRaw},
                    'daily': {input: dailyRateInput, raw: dailyRateRaw},
                    'weekly': {input: weeklyRateInput, raw: weeklyRateRaw},
                    'semi': {input: semiMonthlyRateInput, raw: semiMonthlyRateRaw},
                    'basic': {input: basicSalaryInput, raw: basicSalaryRaw}
                };
                
                fields.forEach(field => {
                    if (field !== currentField && inputs[field]) {
                        if (inputs[field].input) inputs[field].input.value = '';
                        if (inputs[field].raw) inputs[field].raw.value = '';
                    }
                });
                
                updateCalculationDisplay(0, 0, 0, 0, 0);
            }
