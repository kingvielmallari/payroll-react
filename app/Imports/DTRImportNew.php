...
class DTRImport implements ToModel, WithHeadingRow, WithValidation, SkipsOnError, SkipsOnFailure
{
     ...
     * (Updated to work with ALL day types - regular days, rest days, holidays)
     */
    private function calculateDynamicWorkingHours(\App\Models\TimeLog $timeLog)
    {
        ...
        // Determine day type for context (though we use same logic for all types)
            ...
            if ($hasFlexibleBreak && !$hasFixedBreak) {
                ...
                if ($hasActualBreakLogs) {
                    ...
                    if ($workStartTime->lt($breakStart) && $workEndTime->gt($breakEnd)) {
                        ...
                        $breakMinutesToDeduct = 0; // Will be handled by splitting calculation
        ...
        // STEP 7: Calculate overtime based on threshold
        $overtimeHours = 0;
        ...
        // STEP 8: Calculate regular hours (total - overtime, capped at standard)
