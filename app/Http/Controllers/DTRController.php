    ...
    public function show($id)
    {
        try {
            ...
            if ($dtrRecord->employee_id) {
                $employeeRecord = DB::table('employees')->where('id', $dtrRecord->employee_id)->first();
                if ($employeeRecord) {
                    $userRecord = DB::table('users')->where('id', $employeeRecord->user_id)->first();
                    $employee = [
                        'id' => $employeeRecord->id,
                        'first_name' => $employeeRecord->first_name ?? '',
                        'last_name' => $employeeRecord->last_name ?? '',
                        'employee_number' => $employeeRecord->employee_number ?? '',
                        'user' => $userRecord ? [
    ...
    public function edit($id)
    {
        ...
        try {
            ...
            if ($dtrRecord->employee_id) {
                ...
                if ($employeeRecord) {
                    $userRecord = DB::table('users')->where('id', $employeeRecord->user_id)->first();
                    $departmentRecord = DB::table('departments')->where('id', $employeeRecord->department_id)->first();
